...
def real_to_pixel(x_real, y_real):
    # Pre-determined constants for the linear mapping
    x_pixel_min = 538
    x_pixel_max = 844.75
    x_real_min = 261.52
    ...
    # Calculate the x_pixel coordinate
    x_pixel = (x_real - x_real_min) * (x_pixel_max - x_pixel_min) / (x_real_max - x_real_min) + x_pixel_min
    # Calculate the y_pixel coordinate
    y_pixel = (y_real - y_real_min) * (y_pixel_max - y_pixel_min) / (y_real_max - y_real_min) + y_pixel_min
...
cap.set(cv2.CAP_PROP_AUTOFOCUS, 0)  # Enable auto-focus
...
rotation_threshold = 25.0  # Set to a suitable angle in degrees# Initialise variables
robot_id = 190 # ArUco marker ID of the robot
aruco_size = 90 # Size of the ArUco marker in mm8/*9
prev_rot = None
prev_x = None
prev_y = None
rot_current = None
...
clearance = 1 # Should be = to robot radius + safety margin

# Define the rotation and translation thresholds
rotation_threshold = 10.0  # Set to a suitable angle in degrees
...
while frame_count < num_frames_to_capture:
    ...
    if ids is not None:
        for marker_id in ids.flatten():
            ...
            rvecs, tvecs, _ = aruco.estimatePoseSingleMarkers(corners1[index], aruco_size, CM, dist_coef)
        ...
        for contour in contours:
            ...
            for point in approx:
                for ides in all_ids:
                    ...
                    aruco_x, aruco_y = get_coords(ides, initial_average_tvecs)

                    #print(x_robo, y_robo)
                    if abs(x_real - aruco_x) <= thresh and abs(y_real - aruco_y) <= thresh:
                        close_to_aruco = True
                        break
                if not close_to_aruco:
