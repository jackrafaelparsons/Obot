...
def send_angle(angle2motor):
    ...
    client.publish("Obot/movement", str(turn_direction)) # Send value for movement
...
# function to recieve messages from MQTT
...
while(True):
    ...
    if ids is not None and 46 in ids:
        ...
        # Send angle and direction to robot
        # Check if robot is off course to desried coordinate
        if angle_2_motor < upper_bound and angle_2_motor > lower_bound:
            # Check if obstacle has been detected
            send_direction(req_distance)
        else:
            send_angle(angle_2_motor)
