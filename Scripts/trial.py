...
def current_station(next_station, x_current, y_current, current_angle, angle4turn, initial_average_tvecs):
    ...
    if angle4turn > 180:
        Back_angle = angle4turn -180
    else:
        Back_angle = angle4turn + 180

    diff1 = abs(relative_angle - angle4turn)
    diff2 = abs(relative_angle - Back_angle)
    ...
    print(turn_direction, angle_2_motor)

    # Calculate direction of robot

    # calculate upper and lower threshold
    upper_threshold = abs(relative_angle) + 90
    if upper_threshold > 360:
        upper_threshold = upper_threshold - 360
    
    lower_threshold = abs(relative_angle) - 90
    if lower_threshold < 0:
        lower_threshold = 360 + lower_threshold
    
    if upper_threshold > current_angle and lower_threshold < current_angle:
        direction = 'forward'
    else:
        direction = 'backward'
...
while(True):
    ...
    if ids is not None and 46 in ids:
        ...
        if current_angle < upper_bound or current_angle > lower_bound:
            ...
            # Assuming you have a distance (in pixels) and an angle (in degrees)
            # Convert the angle to radians
            # angle_radians = math.radians(angle_2_motor)

            # # Calculate the endpoint coordinates
            # x_endpoint = int(x_current + 1000 * math.cos(1.57))
            # y_endpoint = int(y_current + 1000 * math.sin(1.57))
            # x_current = int(x_current)
            # y_current = int(y_current)

            # # Define the line color and thickness
            # line_color = (0, 0, 255)  # BGR color (red in this example)
            # line_thickness = 20

            # # Draw the line on the frame
            # cv2.line(frame, (x_current, y_current), (x_endpoint, y_endpoint), line_color, line_thickness)
