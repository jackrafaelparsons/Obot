...
x_value, y_value = get_coords(190, initial_average_tvecs)
...
while(True):
    ...
    for marker_id in ids.flatten():
        ...
        if marker_id != robot_id:
            ...
            center_x1 = int(sum(marker_corners[:, 0]) / 4)
            center_y1 = int(sum(marker_corners[:, 1]) / 4)
            ...
            cv2.putText(frame, label_text, (center_x1, center_y1), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
    ...
    if id is not None and robot_id in id:
        ...
        # corners, _ , rP = aruco.detectMarkers(adaptive_threshold, aruco_dict)
        ...
        if optimal_intersection is None:
            if abs(req_distance) <= robot_radius * 1.1:
                # initiate station interaction sequence
                station_index += 1
                print(optimal_route[station_index])
                x_value, y_value = get_coords(optimal_route[station_index], initial_average_tvecs)
                req_distance, angle_2_motor = current_station(x_current, y_current, x_value, y_value, current_angle, angle4turn)
        else:
