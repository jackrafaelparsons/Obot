...
clearance = 50 # Should be = to robot radius + safety margin (in pixels)
...
while(True):
    ...
    if id is not None and robot_id in id:
        ...
        label_text = f"Obot | x: {x_current:.2f} | y: {y_current:.2f} | Angle: {angle_2_motor:.2f}"

        # Draw the label next to the marker's ID
        cv2.putText(frame, label_text, (center_x, center_y), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0), 2)

        # Place label to coordinates around obstacle
