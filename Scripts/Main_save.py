    ...
    for obstacle in expanded_obstacles:
        ...
        for i in range(4):
            ...
            if do_lines_intersect(corner1, corner2, robo_coords, station_coords):
                ...
                obstacle_intersected = True
                break  # Exit the loop if an intersection is found
        
        if obstacle_intersected:
            break
        ...
        for row in point:
            for x, y in row:
                for ids in route:
                    ...
                    if not (x_real > aruco_x - thresh and x_real < aruco_x + thresh and y_real > aruco_y - thresh
                             and y_real < aruco_y + thresh):
...
while(True):
    ...
    if id is not None and robot_id in id:
        ...
        if direction == 1 or direction == 2:
            # if optimal_intersection is None:
            if angle_2_motor > rotation_threshold:
                direction = 0
                client.publish("Obot/movement", str(direction))
