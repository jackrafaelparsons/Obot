...
# function to check if angle is within a 180 deg range of target
...
def obstructed(expanded_obstacles, center_x, center_y, station_pixel_x, station_pixel_y,x_current, y_current, x_value, y_value):
    optimal_intersection = None
    robo_coords = (center_x, center_y)
    station_coords = (station_pixel_x, station_pixel_y)
...
# Function to find average of all accumulated approximations
def obstacle_finder(all_accumulated_approx, route, tolerance=20):
    ...
    obstacles = average_matrix.astype(int)
...
clearance = 60 # Should be = to robot radius + safety margin (in pixels)
...
centers = []
# Loop through the ArUco data and calculate the center for each marker
for corners in corners1:
...
if obstacle_detection:
    ...
    while frame_count < num_frames_to_capture:
        ...
        # undistorted_image = cv2.undistort(frame, CM, dist_coef)
        ...
        for contour in contours:
            ...
            for point in approx:
                for center in centers:
                    x, y = point[0]
                    aruco_x, aruco_y = center[0]

                    if abs(x - aruco_x) <= thresh and abs(y - aruco_y) <= thresh:
                        close_to_aruco = True
                        break
                
                # Filter out Aruco markers and small shapes
                if not close_to_aruco and shape_area > 1000:
                    valid_points.append(approx)
            
    all_accumulated_approx = np.array(valid_points)
    # Find obstacles
...
while(True):
    ...
    if id is not None and robot_id in id:
        ...
        # Extract X and Y coordinates from TVECS
        x_current = tvecs[0, 0, 0]
        y_current = tvecs[0, 0, 1]
        # Transform rvecs into a rotation matrix
        rot = cv2.Rodrigues(np.asarray(rvecs))
        rot = rot[0]
        # transform rotation matrix to angle
        ...
        # Extract the corner positions of the robot (pixels)
        marker_corners = corners[index_robot][0]

        # Calculate the center point of the marker (pixels)
        center_x = int(sum(marker_corners[:, 0]) / 4)
        center_y = int(sum(marker_corners[:, 1]) / 4)
        ...
        if critical_event:
            # detect which phase of critical event is active, assign relevant coordinates
            ...
            if ramp_index == 0:
                ...
                # Allow 5 seconds to pass from turning
        ...
        # If not critical, continue with normal operation
            ...
            if obstacle_index < 1:
                # Extract next stations pixel coordinates
                current_markerID = optimal_route[station_index]
                next_station_corners = corners1[list(ids).index(current_markerID)][0]
                # Calculate the center point of the marker
                station_pixel_x = int(sum(next_station_corners[:, 0]) / 4)
                station_pixel_y = int(sum(next_station_corners[:, 1]) / 4)
                # Check if any obstacles between robot and station in pixels
                optimal_intersection = obstructed(expanded_obstacles, center_x, center_y, station_pixel_x, station_pixel_y,
                                                  x_current, y_current, x_value, y_value)
            
            # check if obstacle is obstructing path
            if optimal_intersection is None:
                # Choose optimum intersection as next objective
                req_distance, angle_2_motor = current_station(x_current, y_current, x_value, y_value, 
                                                              current_angle, angle4turn)
                ...
                if direction == 0:
                    ...
                    if abs(req_distance) <= robot_radius * 1.1:
                        direction = 0
                        client.publish("Obot/movement", str(direction))
                        obstacle_index = 0 # Resets obstacle detection
                        # Flash LEDs
                        i = 0
                        for i in range(4):
                ...
                if direction == 1 or direction == 2:
                    ...
                    # if not at station, check if angle is small enough to move forward
                            ...
                            direction = 0
                            client.publish("Obot/movement", str(direction))
            # No obstacle detected, carry on as normal
            else:
                x_opt = optimal_intersection[0]
                y_opt = optimal_intersection[1]
                print(optimal_intersection)
                obstacle_index = 1 # Stops continual relcalculation of intersection
                # Convert the pixel coordinates to real world coordinates
                x_opt_real, y_opt_real = pixel_to_real(x_opt, y_opt)
                req_distance, angle_2_motor = current_station(x_current, y_current, x_opt_real, y_opt_real, current_angle, angle4turn)
                # Check if robot is still
                if direction == 0:
                    # first check if robot is at station
                    if abs(req_distance) <= ramp_threshold:
                ...
                if direction == 1 or direction == 2:
                    ...
                    if abs(req_distance) <= robot_radius * 1.1:
                        ...
                        optimal_intersection = None
                    # if not at station, check if angle is small enough to move forward
                    elif angle_2_motor > ramp_angle:
                            direction = 0
                            client.publish("Obot/movement", str(direction))

        label_text = f"Obot | x: {angle_2_motor:.2f} | Angle: {angle_2_motor:.2f}"

        # Draw the label next to Obot
        cv2.putText(frame, label_text, (center_x, center_y), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0), 2)

        # Place arrow to coordinates around obstacle
        if optimal_intersection is not None:
            end_x = int(optimal_intersection[0])
            end_y = int(optimal_intersection[1])
        elif critical_event:
            # Place arrow to ramp coordinates
            if ramp_index == 0 or ramp_index == 2:
            ...
            # place arrow to fuel station coordinates
            else:
                next_station_corners = corners1[list(ids).index(fuel_id)][0]
                # Calculate the center point of the marker
                end_x = int(sum(next_station_corners[:, 0]) / 4)
                end_y = int(sum(next_station_corners[:, 1]) / 4)
        # Place arrow to next station coordinates within optimum path route
        else:
            current_markerID = optimal_route[station_index]
            next_station_corners = corners1[list(ids).index(current_markerID)][0]
            # Calculate the center point of the marker
        ...
        # draw arrow from Obot to next target coordinates
